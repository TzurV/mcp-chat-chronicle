...
## Objective
Wire the accepted ChatGPT, Claude, and OpenAI Codex adapters into the public CLI so a user can ingest one supported source and inspect basic archive statistics.

This work package makes `chronicle ingest <path> --provider auto` and `chronicle stats` functional using the WP-1.1 DB layer and the concrete WP-1.2/WP-1.3/WP-1.3.2 adapters.
...
If WP-1.3.2 is not accepted yet, return `blocked` and state that the OpenAI Codex extractor dependency is missing.
...
- `md/handoffs/WP-1.3.2-openai-codex-local-extractor.md`
- `md/handoffs/reports/WP-1.3.2-validation-review.md` once it exists
...
- Provider auto-detection for OpenAI Codex local JSONL/session directories.
...
- Gemini, Claude Code, Cursor, or other Class B importers.
...
- OpenAI Codex local store:
  - a `.jsonl` file where records have `type` and `payload`, including `session_meta` or Codex `response_item` rows;
  - a directory containing nested `rollout-*.jsonl` files under `sessions/`;
  - a Codex home directory containing `session_index.jsonl` and `sessions/`.
...
- Codex `.jsonl` session file;
- Codex session directory or Codex home directory.

Detection rules:

- If provider is explicitly `chatgpt`, call the ChatGPT importer.
- If provider is explicitly `claude`, call the Claude importer.
- If provider is explicitly `openai_codex`, call the OpenAI Codex extractor.
- If provider is `auto`, inspect only enough data to choose ChatGPT, Claude, or OpenAI Codex.
...
- `source_type = "official_export"` for ChatGPT and Claude
- `source_type = "local_store"` for OpenAI Codex
- `provider = "chatgpt"`, `"claude"`, or `"openai_codex"`
...
- `tests/fixtures/openai_codex/` once WP-1.3.2 exists
...
- `chronicle ingest <codex-jsonl-or-dir> --provider auto --db-path <tmp>` detects OpenAI Codex and inserts conversations/messages.
- Explicit `--provider chatgpt` works.
- Explicit `--provider claude` works.
- Explicit `--provider openai_codex` works.
...
- Existing WP-0.1, WP-1.1, WP-1.2, WP-1.3, WP-1.3.1, and WP-1.3.2 tests still pass.

## Acceptance Criteria
WP-1.4 is complete only when all of these are true:

- `chronicle ingest` is functional for ChatGPT and Claude official exports plus OpenAI Codex local sessions.
- `--provider auto` detects ChatGPT, Claude, and OpenAI Codex by file signature.
- Explicit `--provider chatgpt`, `--provider claude`, and `--provider openai_codex` work.
...
- Dependency confirmation that WP-1.2, WP-1.3, WP-1.3.1, and WP-1.3.2 validation reports exist and are accepted.
- Exact command output or concise pasted result for:
  ...
  - at least one OpenAI Codex ingest command against a temp/project-local DB;
...
- Preserve accepted ChatGPT, Claude, and OpenAI Codex parser/extractor behavior.
...
## Dependency Check
Confirm WP-1.2, WP-1.3, WP-1.3.1, and WP-1.3.2 are accepted, with validation report paths.
...
| --- | --- | --- |
| `chronicle ingest` functional for ChatGPT, Claude, and OpenAI Codex | pass/fail/not attempted | Command/test evidence |
| `--provider auto` detects all three providers | pass/fail/not attempted | Test evidence |
| Explicit providers work for all three providers | pass/fail/not attempted | Test evidence |
...
| Did not add Gemini/Claude Code/Cursor/other Class B importers | pass/fail |  |
