...
| Current milestone state | M0 complete; WP-1.1 accepted; M1 importers next |
| Next action | Execute WP-1.2 ChatGPT export importer |
| Current branch | `main` |
...
| WP-1.2 | ChatGPT export importer | Handoff ready | `md/handoffs/WP-1.2-chatgpt-export-importer.md` | `md/handoffs/reports/WP-1.2-completion-report.md` | Pending | Depends on WP-1.1. Must remain concrete code; no adapter base yet. |
| WP-1.3 | Claude export importer | Ready after WP-1.2 or parallel with fixture prep | Pending | Pending | Pending | Depends on WP-1.1. First task is format verification and sanitized synthetic fixture. |
| WP-1.4 | CLI ingest + stats | Blocked by dependencies | Pending | Pending | Pending | Depends on WP-1.2 and WP-1.3. |
...
Send `md/handoffs/WP-1.2-chatgpt-export-importer.md` to the executor chat. The completion report location is:
...
- parse `conversations.json` from official ChatGPT export files;
- handle the tree-shaped `mapping` with regenerated/edited branches;
- produce normalized `Conversation` and `Message` objects using the WP-1.1 model layer;
- log malformed records to `errors_json` without aborting whole ingest runs;
