...
| Current milestone state | M0/M1/M2 core and real-history prototype accepted; OpenAI Codex and Claude Code local extraction accepted; Cursor deferred; M4 MCP not started; WP-5.1 accepted after two rework cycles; WP-5.1.1 accepted after one evidence-integrity rework; WP-5.1.2 private teacher-reference pilot is next |
| Next action | Execute the gated 30-conversation WP-5.1.2 pilot from the approved handoff; expand toward 300 only after its privacy, quality, and configured cost gates pass |
| Current branch | `main` |
