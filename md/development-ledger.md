...
| Current milestone state | M0/M1/M2 core and real-history prototype accepted; Chat Chronicle v0.1.0 is public; hosted candidate generation, provider-generic provenance, disclosure gates, fixed primary-judge configuration, and the bounded Llama/Gemini comparison are accepted |
| Next action | Agree the scope of a larger bounded development pilot before the complete 120-case run; retain Gemini 3.1 Pro Preview with rubric v1 as the primary judge and use Gemini 2.5 Flash only for diagnostic sensitivity |
| Current branch | `main` |
...
7. Agree and run a somewhat larger bounded pilot before all 120 development cases, retaining rubric v1 unless a separately versioned calibration decision is approved. Consider a provider-independent `bench compare` command before a multi-model matrix.
8. Defer remote-machine setup and the separate untouched evaluation set to WP-5.2C decisions.
9. Keep this thread as the main development planning and PM-validation record.
