...
| Last accepted delivery | WP-5.2B1.2 hosted candidate and fixed-Pro-judge eight-case comparison |
| Current milestone state | M0/M1/M2 core and real-history prototype accepted; Chat Chronicle v0.1.0 is public; hosted candidate generation, provider-generic provenance, disclosure gates, fixed primary-judge configuration, and the bounded Llama/Gemini comparison are accepted; the local-first path through the 40-case pilot and complete 120-case retained-model comparison is approved |
| Next action | Execute WP-5.2B1.3: run the same-prefix 10-conversation/40-case pilot for the accepted Llama and hosted Gemini arms, then qualify the WP-5.2A5 local candidates one at a time |
| Current branch | `main` |
...
| WP-5.2B | Gemini-judged development evaluation program | In progress; pilot and complete retained-model run approved | See child packages | See child packages | See child packages | Split generation/local scoring is implemented and the bounded private Gemini smoke is accepted. The owner approved a 40-case pilot followed by all 120 development cases for every retained local candidate before article publication. |
...
| WP-5.2B1.3 | Ten-conversation accepted-arm pilot | Handoff ready | `md/handoffs/WP-5.2B1.3-ten-conversation-accepted-arm-pilot.md` | Pending | Pending | Run the accepted local Llama and hosted Gemini candidates over the same frozen first 10 conversations/40 cases each, score with the fixed Pro rubric-v1 judge, prove cache-only replay, and measure local runtime before WP-5.2A5 qualification. |
| WP-5.2A5 | Additional LM Studio/OpenAI-compatible candidates | Planned; follows 40-case accepted-arm pilot | Pending | Pending | Pending | Qualify Qwen3.5-4B, Phi-4-mini-instruct, Llama 3.2 3B Instruct, and the approved Gemma candidate one at a time on one synthetic and one frozen conversation before admitting them to the common 40-case pilot. |
| WP-5.2B2 | Complete comparative development evaluation | Approved; awaits pilot and candidate qualification | Pending | Pending | Pending | Run every retained local candidate over all 30 frozen conversations/120 cases with the fixed Pro rubric-v1 judge before the technical article. Select publication metrics only after complete private results exist. |
...
7. Run the same frozen-prefix 10-conversation/40-case pilot for the accepted Llama and hosted Gemini candidates without changing prompts, schemas, references, or rubric v1.
8. Qualify each WP-5.2A5 local candidate on all four tasks over one synthetic and one frozen conversation; run passing candidates through the same 40 cases.
9. Run every retained local candidate over all 30 conversations/120 cases before publishing the technical comparison article. Choose the publication metric subset after complete results exist.
10. Run locally by default. Use remote generation only if measured runtime threatens the intended schedule; defer the separate untouched evaluation set to a later WP-5.2C decision.
11. Keep this thread as the main development planning and PM-validation record.
