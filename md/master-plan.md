...
**Status:** In development · real-history core prototype accepted; ingest/collect/stats/search/phrase/recent/open are working across the owner's real ChatGPT, Claude, OpenAI Codex, and Claude Code histories · WP-5.1 configurable AI-task foundation, WP-5.1.1 production task contracts, and WP-5.1.3 local LM Studio compatibility/smoke are accepted · all four initial tasks now execute through local Qwen3.5-4B on short real conversations · WP-5.1.4 Windows CI portability patch is locally accepted and awaits PM commit, owner push, and hosted Windows/Ubuntu confirmation · WP-5.1.2 evaluation remains paused until CI is green and real-data prompts/reference workflow are calibrated · **Plan v3.3 ("Plan A+ — Batch-first, Pluggable Collectors", + configurable AI tasks and measured local-model evaluation)**, **amended by `md/change-order-01.md`** (prototype fast path with Claude Code extractor pulled forward, link-back schema migration, rename to WorkTrail — read both documents). Full reasoning chain in Appendix A. · **Last updated:** 2026-07-15
...
**WP-5.1.4 Windows CI Rich-output wrapping patch.** *(immediate CI patch; handoff at `md/handoffs/WP-5.1.4-windows-ci-rich-output-wrapping.md`)*
...
**WP-5.1.2 Real-data teacher-reference corpus.** *(depends accepted WP-5.1 and completed WP-5.1.1 task contracts; handoff prepared at `md/handoffs/WP-5.1.2-real-data-teacher-reference-corpus.md`)*
*Objective:* Build a private, reproducible evaluation corpus from the owner's real repo-local archive and generate automated frontier-model references for all four WP-5.1.1 tasks. This is a teacher-reference or silver corpus, not human-verified ground truth; no human review is planned in this phase.
*Tasks:* Create a consistent SQLite-native frozen copy of the current product DB under git-ignored `.chronicle/eval/source/`, record a private snapshot manifest/hash, and use only that read-only immutable basis for corpus development and evaluation. Keep canonical evaluation state in `.chronicle/eval/eval.db`; preserve immutable task inputs, source hashes, split/stratum metadata, prompt/schema/model provenance, teacher outputs, reconciliation status, remote-disclosure records, and later candidate predictions. Run a 30-conversation pilot before a deterministic 300-conversation corpus split 150 development / 50 validation / 100 frozen holdout, subject to eligible unique conversations and a configured remote-cost ceiling. Generate independent structured references with explicitly authorized GPT-5.6 Sol and Claude Fable 5 profiles through the WP-5.1 LiteLLM boundary. Implement one bounded dual-provider orchestrator, but require an aggregate-only PM operator checkpoint before the owner manually enters API credentials and launches any real teacher/reconciliation call. Accept direct agreement, machine-reconcile bounded disagreements with recorded provenance, label unresolved disagreements, and exclude unresolved cases from the primary benchmark score. Dates remain deterministic DB facts. Use source-message evidence for factual fields. Detect and quarantine obvious secrets before remote transmission. Store all private inputs, requests, responses, reports, and source mappings locally; only generic tooling, schemas, synthetic tests, and privacy-safe aggregate evidence may be committed.
*AC:* Product DB is unchanged; the evaluation snapshot is created through SQLite's backup API rather than a raw filesystem copy, passes integrity/hash checks, and remains read-only/frozen; corpus selection is seeded, stratified, deduplicated, fingerprinted, resumable, and frozen before candidate testing; the pilot records schema validity, agreement/disagreement, token/cost estimates, latency, retention acknowledgement, and snapshot/source no-write evidence before full-corpus progression; all four tasks have references or explicit failure/disagreement states; executor tooling makes no real remote calls; the PM validates `md/handoffs/reports/WP-5.1.2-operator-checkpoint.md`, then the owner manually launches remote calls with explicit flags and bounded case/token/request/cost limits; no private conversation text, titles, paths, URLs, UUIDs, requests, responses, hashes, manifests, or evaluation DB files are tracked; the detailed completion report is `md/handoffs/reports/WP-5.1.2-completion-report.md`.

**WP-5.2 Benchmark harness.** *(depends WP-5.1.2 frozen corpus — the data behind LP-4 and the article)*
*Tasks:* `bench/run.py`, model-profile × AI-task × prompt-variant matrix using the same `.chronicle/ai-models.yaml` aliases and WP-5.1.1 task contracts as the product. Compare zero-shot and task-specific few-shot prompts; examples may come only from the development split, never validation/holdout, and exposed chain-of-thought is not requested or stored. Core same-size candidates are Qwen3.5-4B, Phi-4-mini-instruct, and Gemma-3-4B-it using a comparable quantization; extended candidates may include Llama 3.2 3B Instruct, SmolLM3 3B, a pinned Ministral 3B-class instruct model, and Qwen3.5-9B as an optional quality ceiling. Treat model artifact/version, quantization, runtime, execution provider, hardware, prompt, and generation settings as separate factors. Include a same-model LM Studio versus Foundry Local runtime comparison where the artifact/runtime formats permit it; Foundry Local's OpenAI-compatible endpoint is a LiteLLM-compatible profile, not a new product inference abstraction. Measure structured-output adherence, summary factual coverage and unsupported claims, classification macro-F1/confusion matrix against teacher references, last-activity factuality/state capture, title-assessment agreement, latency, throughput, token usage, and cost when reported. Report primary scores only on usable teacher-agreement/machine-reconciled cases and publish disagreement/coverage separately.
*AC:* One-command reproducible; `bench/report.py` emits privacy-safe per-task/model/prompt/runtime markdown tables and machine-readable local results; paired comparisons include bootstrap 95% confidence intervals where sample size permits; low-N provider strata are labeled as descriptive; model identity, artifact, quantization, task/prompt/schema versions, hardware, runtime, latency, throughput, and cost are recorded; no holdout case is used for prompt tuning or few-shot examples.
...
**Sequencing & calendar** (~6 focused hrs/week): prototype fast path = WP-1.4 → CO-1 → WP-2.1 → WP-3.1 → real-history demo, now accepted. Claude project metadata, WP-1.5/WP-1.6 usability polish (source inventory, YAML defaults, init, collect), WP-2.3.2 search punctuation hardening, WP-5.1 configurable AI-task infrastructure, WP-5.1.1 production task contracts, and WP-5.1.3 local LM Studio compatibility/smoke are accepted. The active sequence is WP-5.1.4 Windows CI portability patch → manual real-data prompt calibration discussion → revised WP-5.1.2 scope → WP-5.2 local-model benchmark. Do not start the teacher-reference automation until CI is green and the prompt-calibration/reference workflow is agreed. Cursor is deferred; Gemini importer work waits for a real Takeout shape; MCP and rename/release remain later tracks.
...
| Automated teacher references mistaken for ground truth | Name WP-5.1.2 output a silver/teacher-reference corpus; preserve both teacher outputs and disagreement states; exclude unresolved cases from primary scores; publish corpus coverage and teacher agreement with every benchmark |
| Remote evaluation exposes private archive data or incurs unbounded cost | Explicit owner authorization applies only to bounded WP-5.1.2 teacher runs; dry-run manifest, secret quarantine, provider/model allowlist, retention acknowledgement, case/token/cost caps, resumable cache, and local disclosure log are mandatory |
