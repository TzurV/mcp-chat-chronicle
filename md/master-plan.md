...
*Tasks:* Reuse the exact B1 inputs, prompts, portable generation package, local scoring stage, deterministic metrics, fixed `gemini-3.1-pro-preview` rubric-v1 primary judge profile, failure policy, and report layout. Compare schema reliability, evidence validity, categorical agreement and work-mode confusion matrices, Gemini rubric scores, p50/p95 latency, and token/resource data. Keep model quality and runtime performance claims separate when the runtime or quantization differs. Keep any secondary-judge sensitivity analysis separate from primary candidate scoring.
*AC:* Candidate runs reconcile to the same 120 case fingerprints; differences in model, artifact, quantization, runtime, context, or settings are explicit; no comparative claim silently excludes failures; private data remains local and development results are not presented as final evaluation.
