...
**Status:** In development · M0/WP-0.1, M1/WP-1.1, and M1/WP-1.2 accepted; M1/WP-1.3 is next · **Plan v3.1 ("Plan A+ — Batch-first, Pluggable Collectors", + AI-deepening roadmap)** adopted after two architecture reversals and an AI-enrichment review; the full reasoning chain is preserved in Appendix A. · **Last updated:** 2026-07-10
...
- **Collector** — `chronicle collect`: discovers configured sources and runs the right importer/extractor.
...
- Coding-agent conversations have **no URLs** — `chronicle open` shows a local transcript view for those; deep links exist only for web-chat sources.
- "Scheduled collection" = a documented one-line **Windows Task Scheduler** entry invoking `chronicle collect`. No daemon, no service.
...
```bash
chronicle ingest path/to/export.zip --provider auto   # Class A, one file
chronicle ingest-folder path/to/exports               # sweep a drop folder
chronicle collect                                     # run all enabled sources
chronicle scan-local                                  # read-only: what sources exist on this machine?
chronicle stats                                       # counts per source, last runs, unjoined/errors
chronicle search "docker network"                     # FTS5, ranked, snippets, links
chronicle open <result-id>                            # deep link (web) or transcript view (coding agents)
```
...
Deep links: ChatGPT `https://chatgpt.com/c/{conv_id}`, Claude `https://claude.ai/chat/{uuid}` (both IDs in official exports). Gemini and all coding agents: `url` NULL → `chronicle open` renders the stored transcript.

---

## 6. Milestones & Work Packages

Each WP = one sub-agent handoff: **Objective · Tasks · Acceptance criteria (AC) · Notes/sources.** Build order: schema → Class A importers → CLI/search → collect workflow → Class B extractors → MCP recall (time-fenced) → enrichment/benchmark → release.
...
*AC:* Runs in <2 s; never touches file contents beyond existence/mtime; output matches §4 shape.

**WP-1.6 collect + folder workflow + scheduling docs.**
*Objective:* The recurring usage loop.
*Tasks:* `sources` CRUD via CLI (`chronicle source add/list/disable`); `chronicle ingest-folder` sweeps a drop folder for export zips; `chronicle collect` iterates enabled sources → adapters → `ingest_runs`; docs page: one-line **Windows Task Scheduler** setup for nightly `chronicle collect` (no daemon — design constraint §1.6).
...
*Tasks:* FTS5 `MATCH` + `bm25()` ranking; filters `--provider --since --until --tag`; Rich table: date · source · title · snippet() · link/id; `chronicle open <id>` → launch URL (web sources) or render stored transcript with Rich paging (coding agents/Gemini).
*AC:* p95 < 100 ms on 5k conversations; quoting/empty-query edge cases handled; `open` works for both URL and URL-less rows.
...
*Tasks:* **First task is a research spike** — inventory the actual JSONL structure on the developer's machine and freeze it into a fixture + short format memo in `md/` (the format is undocumented; the memo is also LP-3 material). Then: map sessions → conversations (project dir + session id), messages with roles/timestamps; incremental by file mtime + content hash.
*AC:* Fixture round-trips; re-collect with no new sessions → 0 changes; unknown record types logged and skipped.
...
*Tasks:* `mcp_server.py`, stdio transport, `chronicle serve`. Tools (docstrings matter — the model reads them):
...
*Tasks:* `chronicle search --smart` (and an MCP parameter): local model rewrites a vague query into FTS terms + date hints (structured output); hard 2 s timeout → raw-query fallback. **Not on by default** — no LLM call on the default search path.
...
*Tasks:* `chronicle digest [--days 7]`: gather the window's conversations (all sources) → summaries + knowledge items → one structured local-model pass → markdown digest (themes, per-project activity, open questions carried forward, links/ids); write to stdout + `md/digests/`; document a weekly Task Scheduler line next to the nightly `collect` docs.
...
### M6 — Release & polish
**WP-6.1** README (positioning §1.4, two-tier quickstart, architecture diagram, honest limitations incl. export cadence for web chats) · demo GIF (search hitting ChatGPT + Claude Code + Cursor results in one query) · `v0.1.0` tag · PyPI (`pipx install chat-chronicle`).
*AC:* A stranger on Windows 11: `pipx install` → first successful search < 5 min with README only. **Install tiers:** Tier 1 search-only ≈ 5 min (zero AI deps); Tier 2 + LM Studio + `chronicle setup` (writes Claude Desktop MCP config, pings LM Studio) ≈ 15 min. No broken states — only leaner (design rule 3).
...
**LP-1 (after M2) — "I asked an AI to design my project. Its architecture was impossible."**
Hook: the original AI-generated plan assumed MCP clients passively stream telemetry — they don't, and models don't know their own conversation IDs. Content: the three-stage evolution (impossible telemetry → clever marker workaround, designed then shelved → boring batch-first adapters), and why "the boring architecture is the one that ships." Visual: the A→B→A+ evolution diagram. CTA: "What's the cleverest design you've ever cut?"

**LP-2 (after M2) — "SQLite FTS5 is criminally underrated."**
The whole search layer is stdlib + one virtual table: bm25, porter tokenizer, snippet(); p95 latency on a real archive. Visual: terminal GIF of `chronicle search`. Code-forward — the post developers save.

**LP-3 (after M3) — "Your AI coding tools keep full transcripts on your disk. Here's where."**
The forensics post. Content: `chronicle scan-local` output; Claude Code's JSONL in `~/.claude/projects`, Cursor's workspaceStorage SQLite; the Class A/B/C source model; copy-before-read and format-drift defenses; privacy implications (it's *your* data — index it). Visual: scan-local screenshot. High curiosity factor.
...
**Class B stores:** undocumented — WP-3.1/3.2 research spikes produce the authoritative format memos in `md/` (verify paths/format on the dev machine; do not trust third-party blog posts over local inspection)
...
**Prior art:** [OpenMemory MCP](https://mem0.ai/blog/introducing-openmemory-mcp) · [OpenChat](https://github.com/p0u4a/openchat) · [LLMLingua](https://www.llmlingua.com/) · [RouteLLM](https://github.com/lm-sys/RouteLLM)
