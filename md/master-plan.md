...
**Sequencing & calendar** (~6 focused hrs/week): prototype fast path = WP-1.4 → CO-1 → WP-2.1 → WP-3.1 → real-history demo, now accepted. Claude project metadata, WP-1.5/WP-1.6 usability polish (source inventory, YAML defaults, init, collect), WP-2.3.2 search punctuation hardening, WP-5.1 configurable AI-task infrastructure, WP-5.1.1 production task contracts, WP-5.1.3 local LM Studio compatibility/smoke, and WP-5.1.4 CI portability are accepted. WP-5.1.2 and WP-5.2 are paused. The active workstream is first-release preparation in a separate release thread using `md/release-1-planning-note.md`; this main thread remains the development-manager record. Cursor is deferred; Gemini importer work waits for a real Takeout shape; MCP remains a later track unless explicitly reprioritized.
