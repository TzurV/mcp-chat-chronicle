...
Use `gemini-2.5-flash` as the initial development judge, called through LiteLLM. Select it through a local YAML judge-profile alias rather than hard-coding the provider/model in evaluation logic; API credentials remain environment variables. Persist the logical alias, resolved provider/model identifier, rubric version, generation settings, request identity, and failure state. The judge receives the frozen selected source, FABLE silver reference, and schema-valid candidate result without candidate-model identity, and scores the candidate against the source rather than textual similarity to FABLE. Use task-specific structured rubrics for factual consistency, material coverage, unsupported claims, work-mode reasoning, recent-state/blocker/next-action quality, and title-fit/suggestion quality. Keep deterministic and judge metrics separate and do not publish a single composite score in the first version. Skip semantic judging for schema-invalid outputs because the product cannot consume them, while preserving their raw responses locally for diagnosis. FABLE remains the teacher and `gemini-2.5-flash` the initial judge; neither is ground truth and no human review is planned for this development set.
...
*Tasks:* Reuse the exact B1 inputs, prompts, portable generation package, local scoring stage, deterministic metrics, `gemini-2.5-flash` judge profile/rubric, failure policy, and report layout. Compare schema reliability, evidence validity, categorical agreement and work-mode confusion matrices, Gemini rubric scores, p50/p95 latency, and token/resource data. Keep model quality and runtime performance claims separate when the runtime or quantization differs.
...
| LLM-as-judge bias or instability | Use locally configured `gemini-2.5-flash` as the initial judge through a separately configured LiteLLM profile; record the resolved model; version the rubric and judge settings; blind candidate identity; judge against source rather than FABLE wording; keep deterministic and judge metrics separate; preserve failures and score coverage |
