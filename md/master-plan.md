...
**Status:** Chat Chronicle v0.1.0 source release published · release commit `1f3fbce` and remote tag `v0.1.0` are available from the public GitHub repository/release page · real-history core prototype accepted; ingest/collect/stats/search/phrase/recent/open work across the owner's real ChatGPT, Claude, OpenAI Codex, and Claude Code histories · WP-5.1 configurable AI-task foundation, WP-5.1.1 production task contracts, WP-5.1.3 local LM Studio compatibility/smoke, WP-5.1.4 Windows CI portability, the frozen 30-conversation x four-task FABLE development corpus, and WP-5.2A1 Llama 3.2 1B evaluation-floor integration are accepted · WP-5.2B1, WP-5.2B1.1, WP-5.2B1.2, and WP-5.2B1.3 are accepted; the same-prefix 10-conversation/40-case pilot produced 21/40 schema-valid Llama 3.2 1B outputs and 39/40 hosted Gemini 3.5 Flash outputs, with every eligible result judged and both cache-only replays making zero calls · Gemini 3.1 Pro Preview is pinned in tracked defaults as the fixed primary rubric-v1 judge with temperature 0, a 1,000-token cap, and no reasoning · WP-5.2B1.4 is the next approved local-first run: complete 120-case Gemini baseline, complete 120-case Llama floor, then Qwen3.5-4B 40-case operational checkpoint and complete 120-case control; the intended 400 candidate cases may run unattended overnight with resumable private artifacts · Qwen model-quality failures do not block its complete run when transport, provenance, package, and accounting remain valid · remote local-model generation is a fallback only if measured runtime becomes impractical · article metric selection and presentation remain later decisions · remaining WP-5.2A5 models follow after these real complete-scope numbers · Chrome Gemini Nano remains a separate optional adapter track; remote machine and later untouched evaluation set remain TBD · **Plan v3.6 ("Plan A+ — Batch-first, Pluggable Collectors", + configurable AI tasks and measured local-model evaluation)**, **amended by `md/change-order-01.md`**. Full reasoning chain in Appendix A. · **Last updated:** 2026-07-22
...
**WP-5.2B1.4 Three-candidate 120-case overnight run.** *(handoff ready at `md/handoffs/WP-5.2B1.4-three-candidate-120-case-overnight-run.md`; depends accepted WP-5.2B1.3)*
...
**Sequencing & calendar** (~6 focused hrs/week): prototype fast path = WP-1.4 → CO-1 → WP-2.1 → WP-3.1 → real-history demo, accepted. Claude project metadata, WP-1.5/WP-1.6 usability polish, WP-2.3.2 search punctuation hardening, WP-5.1 configurable AI-task infrastructure, WP-5.1.1 production task contracts, WP-5.1.3 local LM Studio compatibility/smoke, WP-5.1.4 CI portability, and the public v0.1.0 source release are accepted. The resumed AI-development sequence is WP-5.1.2A frozen snapshot → WP-5.1.2B direct FABLE references → WP-5.2A1 Llama 3.2 1B floor integration → WP-5.2B1 split local/hosted evaluation harness with fixed `gemini-3.1-pro-preview` rubric-v1 primary judge → accepted same-prefix 40-case Llama/Gemini pilot → WP-5.2B1.4 complete 120-case Gemini baseline and Llama floor plus Qwen 40/120 control → remaining WP-5.2A5 local candidates → common complete-scope WP-5.2B2 comparison → technical LinkedIn article. Run locally by default and use an ignored resumable overnight workflow; activate remote local-model generation only if measured runtime becomes impractical. Gemini 2.5 Flash remains diagnostic judge-sensitivity evidence only. Select the article metric subset after complete results are available. WP-5.2A2 Chrome Gemini Nano feasibility may run after the article-critical HTTP-model sequence; WP-5.2A3 implementation requires separate approval. WP-5.2A4 hosts remain optional. A separate untouched evaluation set follows only after prompts and models are frozen. Cursor, Gemini history import, MCP, WorkTrail rename, and PyPI publication remain later work unless explicitly reprioritized.
