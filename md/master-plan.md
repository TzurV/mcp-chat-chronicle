...
**Status:** Chat Chronicle v0.1.0 source release published · release commit `1f3fbce` and remote tag `v0.1.0` are available from the public GitHub repository/release page · real-history core prototype accepted; ingest/collect/stats/search/phrase/recent/open work across the owner's real ChatGPT, Claude, OpenAI Codex, and Claude Code histories · WP-5.1 configurable AI-task foundation, WP-5.1.1 production task contracts, WP-5.1.3 local LM Studio compatibility/smoke, WP-5.1.4 Windows CI portability, the frozen 30-conversation x four-task FABLE development corpus, and WP-5.2A1 Llama 3.2 1B evaluation-floor integration are accepted · WP-5.2B1 through WP-5.2B1.4 are accepted · the complete same-case development run accounted for all 400 candidate positions: Gemini 3.5 Flash produced 112/120 schema-valid outputs, Qwen3.5-4B produced 84/120, Llama 3.2 1B produced 57/120, and the separate Qwen checkpoint produced 34/40; every eligible output has a terminal fixed-Pro outcome and all four cache-only replays made zero calls · WP-5.2A5.1 is accepted: Phi-4 Mini, Llama 3.2 3B, and Gemma 3 4B each passed the fixed 8/8 qualification matrix, while Gemma 4 was rejected at the pinned runtime/structured-output compatibility gate · WP-5.2B2.1 is accepted: Phi and Llama 3B each produced 26/40 schema-valid outputs, Gemma 3 produced 23/40, and fixed-Pro recovery completed 74/75 eligible scores with one retained Gemma judge failure · Gemini 3.1 Pro Preview remains the fixed primary rubric-v1 judge with temperature 0, a 1,000-token cap, and no reasoning · local Qwen required about 4h43m for the complete arm on the recorded laptop class, so local overnight execution is practical but operationally slow · Phi, Llama 3B, and research-comparator Gemma 3 proceed next to complete 120-case WP-5.2B2.2 arms before article finalization · Chrome Gemini Nano remains a separate optional adapter track; remote machine and later untouched evaluation set remain TBD · **Plan v3.6 ("Plan A+ — Batch-first, Pluggable Collectors", + configurable AI tasks and measured local-model evaluation)**, **amended by `md/change-order-01.md`**. Full reasoning chain in Appendix A. · **Last updated:** 2026-07-23
...
**WP-5.2B2.2 Three retained candidate complete arms.** *(handoff ready at `md/handoffs/WP-5.2B2.2-three-retained-candidate-complete-arms.md`; depends accepted WP-5.2B2.1)*
...
**WP-5.2B3 Prompt-strategy development study.** *(proposed backlog; begins only after baseline model comparison and separate owner approval)*
*Objective:* Test whether versioned prompt strategies let the best one or two local models approach the unchanged Gemini-120 cloud control on the four product tasks.
...
**Sequencing & calendar** (~6 focused hrs/week): prototype fast path = WP-1.4 → CO-1 → WP-2.1 → WP-3.1 → real-history demo, accepted. Claude project metadata, WP-1.5/WP-1.6 usability polish, WP-2.3.2 search punctuation hardening, WP-5.1 configurable AI-task infrastructure, WP-5.1.1 production task contracts, WP-5.1.3 local LM Studio compatibility/smoke, WP-5.1.4 CI portability, and the public v0.1.0 source release are accepted. The resumed AI-development sequence through WP-5.2B1.4 is accepted: frozen snapshot → direct FABLE references → Llama floor integration → split local/hosted harness and fixed Pro judge → same-prefix 40-case pilot → complete 120-case Gemini, Llama, and Qwen arms plus Qwen checkpoint. WP-5.2A5.1 is also accepted: Phi-4 Mini, Llama 3.2 3B, and Gemma 3 4B qualified; Gemma 4 did not. Next: run their common WP-5.2B2.1 40-case checkpoint → admit retained candidates to complete-scope WP-5.2B2 runs → finalize the LP-4 analysis and publication metric subset → technical LinkedIn article. WP-5.2B3 prompt strategy testing remains a post-baseline backlog item for the best one or two local models. Run locally by default and use an ignored resumable overnight workflow; activate remote local-model generation only if measured runtime becomes impractical. Gemini 2.5 Flash remains diagnostic judge-sensitivity evidence only. WP-5.2A2 Chrome Gemini Nano feasibility may run after the article-critical HTTP-model sequence; WP-5.2A3 implementation requires separate approval. WP-5.2A4 hosts remain optional. A separate untouched evaluation set follows only after prompts and models are frozen. Cursor, Gemini history import, MCP, WorkTrail rename, and PyPI publication remain later work unless explicitly reprioritized.
...
**LP-4 (after every retained local candidate completes the 120-case WP-5.2B2 development run) — "I benchmarked 5 local SLMs on my own laptop for real work. Results surprised me."**
The data post — highest expected reach. Use a later owner-approved subset of the complete WP-5.2 results rather than publishing every collected metric; include structured-output reliability, selected quality and performance measures, exact model/runtime/hardware provenance, and one clean visual. Frame it as an engineering comparison on the owner's frozen real-work development corpus, not a scientific or generally representative leaderboard. Remote generation, if required for schedule reasons, must be clearly separated from local-laptop performance claims.

**LP-4.1 analysis preparation.** *(provisional brief accepted 2026-07-23; handoff at `md/handoffs/LP-4.1-local-model-results-analysis-and-article-planning.md`; brief at `md/handoffs/reports/LP-4.1-local-model-results-analysis-brief.md`; validation at `md/handoffs/reports/LP-4.1-validation-review.md`)*
Use accepted aggregate evidence to discuss task difficulty, task-specific model routing, main observations, public metric selection, and whether a reliability-adjusted composite score is defensible. Treat Gemini 3.5 Flash as the strong hosted control rather than ground truth. Prepare an extensible analysis brief only after owner discussion and approval; do not publish or create final article copy/graphics until the remaining candidate results are incorporated and the owner explicitly approves the editorial plan.
