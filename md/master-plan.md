...
**WP-5.1.1 Initial conversation-intelligence task catalog.** *(depends accepted WP-5.1; handoff ready at `md/handoffs/WP-5.1.1-initial-conversation-intelligence-tasks.md`)*
...
*Tasks:* Keep canonical evaluation state in a separate git-ignored `.chronicle/eval/eval.db`; preserve immutable task inputs, source hashes, split/stratum metadata, prompt/schema/model provenance, teacher outputs, reconciliation status, remote-disclosure records, and later candidate predictions. Run a 30-conversation pilot before a deterministic 300-conversation corpus split 150 development / 50 validation / 100 frozen holdout, subject to eligible unique conversations and a configured remote-cost ceiling. Generate independent structured references with explicitly authorized GPT-5.6 Sol and Claude Fable 5 profiles through the WP-5.1 LiteLLM boundary. Accept direct agreement, machine-reconcile bounded disagreements with recorded provenance, label unresolved disagreements, and exclude unresolved cases from the primary benchmark score. Dates remain deterministic DB facts. Use source-message evidence for factual fields. Detect and quarantine obvious secrets before remote transmission. Store all private inputs, requests, responses, reports, and source mappings locally; only generic tooling, schemas, synthetic tests, and privacy-safe aggregate evidence may be committed.
*AC:* Source DB is opened read-only and unchanged; corpus selection is seeded, stratified, deduplicated, fingerprinted, resumable, and frozen before candidate testing; the pilot records schema validity, agreement/disagreement, token/cost estimates, latency, retention acknowledgement, and zero source-DB writes before full-corpus progression; all four tasks have references or explicit failure/disagreement states; remote calls require explicit flags and bounded case/cost limits despite this WP's owner authorization; no private conversation text, titles, paths, URLs, UUIDs, requests, responses, or evaluation DB files are tracked; the detailed completion report is `md/handoffs/reports/WP-5.1.2-completion-report.md`.
...
**Sequencing & calendar** (~6 focused hrs/week): prototype fast path = WP-1.4 → CO-1 → WP-2.1 → WP-3.1 → real-history demo, now accepted. Claude project metadata, WP-1.5/WP-1.6 usability polish (source inventory, YAML defaults, init, collect), WP-2.3.2 search punctuation hardening, and WP-5.1 configurable AI-task infrastructure are accepted. The active M5 sequence is WP-5.1.1 task contracts → WP-5.1.2 private teacher-reference corpus → WP-5.2 local-model benchmark. WP-5.1.2 starts with a 30-conversation pilot before remote-cost-gated expansion to 300. Cursor is deferred; Gemini importer work waits for a real Takeout shape; MCP and rename/release remain later tracks.
...
**Progress-share checkpoint — Friday, 17 July 2026.** Share the accepted real-history prototype and, if useful, the accepted WP-5.1 YAML/LiteLLM foundation. Public evidence may show the multi-source architecture, supported providers, aggregate counts, privacy-safe terminal output for `stats`, `recent`, or search, and synthetic/config-only AI-task discovery. Redact local usernames, absolute paths, conversation titles, snippets, URLs, UUIDs, transcript text, prompts/results from real histories, and private model/account configuration. Do not present the four WP-5.1.1 production tasks or real-data evaluation as implemented.
